...
class Colony:
    class AntOnRoute:
        def __init__(self, distances, cities, alpha, beta, rho, q0, tau0, pheromones_global, min_nn_heuristic, first_time):
            ...
            self.q0 = q0
            self.tau0 = tau0
            self.pheromones_global = pheromones_global
            self.min_nn_heuristic = min_nn_heuristic
            self.first_time = first_time

            self.sigma = self.rho
            self.pheromones_on_arcs_loc = np.copy(self.pheromones_global)

        def travel_and_pheromones_update(self):
            self.tour_length = 0
            self.tour_current = []
            ...
            self.__find_tour()
            # обновление муравьем феромонов на проложенном им пути
            if not self.first_time:
                self.__local_pheromones_update()
            
            return self.tour_length, np.array(self.tour_current), self.pheromones_on_arcs_loc
        ...
        def __choose_next_city(self):
            ...
            for candidate in range(len(self.non_visited_cities)):
                pheromones_on_arc = self.pheromones_on_arcs_loc[self.current_city][self.non_visited_cities[candidate]]
    ...
    def ants_full_search(self):
        ...
        for iteration in self.iters_count:
            ...
            # внесение на "карту" феромонов изменений, произведенных муравьями при нахождении каждым маршрута
            #self.__pheromones_update_after_ants_travel(tours_of_all_ants)
            # поиск кратчайшего на данной итерации маршрута
            self.__update_optimum(tours_of_all_ants)
        ...
        return self.global_min_tour_length 
                                        ...
                                        #         #                                                                         self.global_min_tour)
    # def __pheromones_update_after_ants_travel(self, tours_of_all_ants):
    #     all_pher_maps = np.empty(0, dtype = 'int32')
    #     for tour in tours_of_all_ants:
    #         all_tours_lengths = np.append(all_tours_lengths, tour[0])   
    #     self.global_min_tour_length = min(all_tours_lengths)
    #     self.global_min_tour = tours_of_all_ants[np.argwhere(all_tours_lengths == self.global_min_tour_length)][1]        
    ...
    def __global_pheromones_update(self):
        ...
        self.pheromones_on_arcs_glob *= (1 - self.sigma)
        for first_city, next_city in zip(self.global_min_tour[:-1], self.global_min_tour[1:]):
            self.pheromones_on_arcs_glob[first_city][next_city] += self.sigma * tau_delta
    ...
    def __update_optimum(self, tours_of_all_ants):
        """
        ...
        """
        all_tours_lengths = np.empty(0, dtype = 'int32')
        for tour in tours_of_all_ants:
            all_tours_lengths = np.append(all_tours_lengths, tour[0])   
        self.global_min_tour_length = min(all_tours_lengths)
        self.global_min_tour = tours_of_all_ants[np.argwhere(all_tours_lengths == self.global_min_tour_length)][1]
...
if __name__ == '__main__':
    ...
    distances = np.random.randint(1,50, (9,9))
    x = Colony(distances)
